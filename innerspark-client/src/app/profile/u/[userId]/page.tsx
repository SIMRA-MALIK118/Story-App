...
export default function PublicProfileByIdPage() {
  ...
  const isOwnProfile = (user as any)?.id === userId;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        // Profile by ID — if this fails, show not found
        const profileRes = await api.get(`/users/id/${userId}`);
        const p = profileRes.data.data.profile;
        if (!p) { setNotFound(true); setLoading(false); return; }
        setProfile(p);

        // Stats + stories can fail independently — don't block the profile
        const [statsRes, storiesRes] = await Promise.allSettled([
          p.username ? api.get(`/users/${p.username}`) : Promise.reject(),
          api.get(`/stories/user/${userId}`),
        ]);

        if (statsRes.status === "fulfilled") {
          const d = statsRes.value.data.data;
          setStats(d.stats || { stories: 0, followers: 0, following: 0 });
          setIsFollowing(d.isFollowing || false);
        }
        if (storiesRes.status === "fulfilled") {
          setStories(storiesRes.value.data.data.stories || []);
        }
      } catch {
        setNotFound(true);
      } finally {
